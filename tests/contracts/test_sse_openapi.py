...
def test_stream_topic_declares_event_stream(openapi_spec):
    spec = openapi_spec
...
def test_location_features_stream_declares_event_stream(openapi_spec):
    spec = openapi_spec
    paths = spec.get("paths", {})
...
def test_v1_stream_declares_event_stream_if_present(openapi_spec):
    spec = openapi_spec
    paths = spec.get("paths", {})
    path = "/api/v1/stream"
    if path in paths:
